...
public class seoul {
	...
	public static String prefix = "seoul";
	 ...
	 *
	 */
	public static String reqHost = "http://www.seoul.co.kr";
	public static int reqPort = 80;
	public static String reqPath = "/news/newsList.php?section=politics";


	public static void getNewsSeoul(int pageno, String dtDate, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws Exception{
		...
		getSum = seoul.parseHtmlSeoul(rsbody, outFwriter, outFwriterReply);
		searchNum += getSum;
		//System.out.println(getDate+"'th News "+ pageno + " PAGE getting count : "+getSum);
	}
	
	public static int parseHtmlSeoul(String rsbody, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws Exception {
		Document doc = Jsoup.parse(rsbody,"UTF-8");
		
		/** get TITLE & LINK List **/
		Elements docTitles = doc.select(".article dt span[id=tit] a");
		...
		/** get DATE List **/
		Elements docDates = doc.select(".article dt span[id=date]");
		//System.out.println("Dates: => \n"+docDates.toString());
		ArrayList<String> rEm = new ArrayList<String>();
		String eDate = "";
		for (Element eD: docDates) {
			eDate = eD.text().toString();
			if (eDate.length() > 0) { eDate = getwww.removeTag(eDate).trim(); }
		...
		for (String se: rEm) {	
			...
			if(rLinks.get(senum) != null && rLinks.get(senum).length() > 1) {
				...
				Map<String,String> contentMap = seoul.getNewsSeoulContent(rLinks.get(senum), encoding);
				...
				*/
				...
				//Map<String,Object> replyMap = Seoul.getNewsSeoulReply(keyset[9].replace(".html",""),1,null, outFwriterReply);
	...
	public static Map<String, String> getNewsSeoulContent(String url, String encoding) throws Exception {
		String rsbody = getwww.getURLBody(url,encoding);
		Map<String,String> content = seoul.parseHtmlSeoulContent(rsbody);
		//System.out.println(url+"'th => \n"+ content.get("content"));		
		return content;
	}	
	
	public static Map<String, String> parseHtmlSeoulContent(String rsbody) throws ParseException {
		...
		try {
		Elements docC1 = doc.select(".atic_txt1");
		content = docC1.text();
	...
	public static Map<String, Object> getNewsSeoulReply(String newsId, int pageno, Map<String, Object> replyMap, OutputStreamWriter outFwriterReply) throws Exception {
		//System.out.println("* Start get reply =================================================================================================");
		String encoding = "euc_kr";
		
		String url = "http://m100.Seoul.com/svc/guest/list.html?article="+newsId;
		int getSum = 0;
		...
		String rsbody = getwww.getURLBody(url,encoding);		
		replyMap = seoul.parseHtmlSeoulReply(rsbody, replyMap);
		...
		if (getSum > 0) { 
			seoul.getNewsSeoulReply(newsId, ++pageno, replyMap, outFwriterReply);  // #TODO 이번 페이지에 읽어온 댓글이 있을 경우 다음 댓글 페이지를 검색하도록 조정 -재귀호출 
		} else {
			// FILE WRITING
	...
	public static Map<String, Object> parseHtmlSeoulReply(String rsbody, Map<String, Object> replyMap) throws ParseException {
		Document doc = Jsoup.parse(rsbody);
		...
		try {
			...
			for (int i=0; i<replyUser.size(); i++) {
				if (!replyComment.get(i).toString().startsWith("관리자가")) {
					...
					reply.add("seoul"+ replyMap.get("newsId").toString() + "\t" + replyUser.get(i).toString() + "\t"   
