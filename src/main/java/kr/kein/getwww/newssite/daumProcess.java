...
import kr.kein.getwww.util.CommonUtils;
...
public class daumProcess extends Thread {
	...
	public static String searchDaumNews(int pageno, String dtDate, String newsCate, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws Exception{
		System.out.println("* Start searchDaumNews =================================================================================================");
		int getSum = 0;		
	...
	public static String parseJsonDaum(String rsbody, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws Exception {
		...
		for (String se: rEm) {	
			...
			if(rLinks.get(senum) != null && rLinks.get(senum).length() > 1) {
				...
				Map<String,String> contentMap = daumProcess.getNewsDaumContent(rLinks.get(senum), encoding);
				...
				String outText = 
						addQt(prefix) 
						+ tab + addQt(uuid )
						+ tab + addQt(getwww.newsCate )
						+ tab + addQt(rDate )
						+ tab + addQt(rcomp )
						+ tab + addQt(author)
						+ tab + addQt(getwww.removeTag(rTitles.get(senum)) )
						+ tab + addQt(rLinks.get(senum) )
						+ tab + addQt(getwww.removeTag(content))
						+ tab + addQt(nId)
						+ tab + addQt(agreeCnt);
				...
				String articleId = daumProcess.getDaumNewsKey(nId, "UTF-8");
				
				Map<String,Object> replyMap = daumProcess.getNewsDaumReply(1, null, outFwriterReply, nId, articleId, "UTF-8");
	...
	public static Map<String, String> getNewsDaumContent(String url, String encoding) throws Exception {
		...
		Map<String,String> content = daumProcess.parseHtmlDaumContent(rsbody);
	...
	public static Map<String, Object> getNewsDaumReply(int pageno, Map<String, Object> replyMap, OutputStreamWriter outFwriterReply, String newsId, String articleId, String charset) throws Exception {
		...
		if (!"".equals(articleId)) {
			...
			replyMap = daumProcess.parseHtmlDaumReply2(rsbody, replyMap, outFwriterReply, newsId);
	...
	public static Map<String, Object> parseHtmlDaumReply2(String rsbody, Map<String, Object> replyMap, OutputStreamWriter outFwriterReply, String newsId) throws ParseException {
		...
		try {
			System.out.println("### replys :: "+rsbody);
		    ...
		    for (int i = 0; i < jArr.size(); i++) {
		    	 ...
		    	 */
		    	if (childCount > 0) {
		    		daumProcess.getDaumReplyReply(1, replyMap, outFwriterReply, newsId, articleId, cmntId, "UTF-8");
		    	}
	...
	public static Map<String, Object> getDaumReplyReply(int pageno, Map<String, Object> replyMap, OutputStreamWriter outFwriterReply, String newsId, String articleId, String commentId, String charset) throws Exception {
		...
		if (!"".equals(articleId)) {
			...
			replyMap = daumProcess.parseHtmlDaumReplyReply(rsbody, replyMap, outFwriterReply, newsId, commentId);
			getSum = Integer.parseInt(replyMap.get("getSum").toString());	
			if(!"file".equals(getwww.runningMode)) {
			...
			if (getSum > 0) {
				replyMap = daumProcess.getDaumReplyReply(++pageno, replyMap, outFwriterReply, newsId, articleId, commentId, charset);
			}				
		}
