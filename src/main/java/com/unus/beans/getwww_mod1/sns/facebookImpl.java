...
public class facebookImpl {
	...
	public static final String dataType = "fb";
	
	public static final String appId = "484180101721290";
	public static final String appSecret = "54fae87d7600e150d4b6fd978fa8c3b7";
	public static final String oldToken = "CAACEdEose0cBAJmKj42WsMkZB3zsJQ75HoBxhQVpUWk3cXELvIFdliwo4voPSi9bBuZCvJXPzR4sgZAafpcAtFZCdd0BwBWyRozkXyCsBpDQMBdTNV55TLgQkqvs7ObMB3ZBE7wuykfrq7eJaRxOi8Hl2rO065za1gctWxRgSmnGjjQELezCtAviPsN0idJbuqvngc30DxLNp9xezhQCrHoYEjZBKMTskZD";
	...
	public static void getFacebookPosts(String searchID, int pageno, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws Exception{
		...
		System.out.println("Facebook getNewAccessToken : "+newAccessToken);
		...
		String facebookID = getFacebookID(searchID);
		
		runFacebookPosts(facebookID, facebook, true, "", outFwriter, outFwriterReply);
		runFacebookLinks(facebookID, facebook, true, "", outFwriter, outFwriterReply);
		runFacebookFeed(facebookID, facebook, true, "", outFwriter, outFwriterReply);
	}
	
	public static String getFacebookID(String searchID) throws FacebookException, JSONException, IOException {

			String idPage = "http://graph.facebook.com/"+searchID;
			String result = getwww.getHtmlBody(idPage, 0, "", null, "UTF-8");
			System.out.println("\n ++ Facebook :: Result of searched ID : "+result + "\n +++ for searchID : "+searchID);
			
			JSONObject resultObj = new JSONObject(result);
			String facebookID = (String) resultObj.get("id");
			System.out.println(" :: "+searchID+"  :> Facebook ID : "+facebookID);
			return facebookID;
	}
	
	public static String getNewAccessToken() throws FacebookException, JSONException, IOException {

		String idPage = "https://graph.facebook.com/oauth/access_token?grant_type=client_credentials&client_id="+appId+"&client_secret="+appSecret;
		String result = getwww.getHtmlBody(idPage, 0, "", null, "UTF-8");

		String newAccessToken = "";
		if (!"".equals(result)) {
			String[] resultStr = result.split("=");
			newAccessToken = resultStr[1];
		}
		
		System.out.println(" :: "+newAccessToken+"  :> Facebook newAccessToken : "+newAccessToken);
		return newAccessToken;
}
	
	public static void runFacebookPosts(String searchID, Facebook facebook, boolean isStart, String nextPage, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws FacebookException, JSONException, IOException {
		...
		if (isStart) { 
			...
			//nextPage = searchID+"/links?fields=id,updated_time,name,caption,description,message,likes&limit=20&offset=1";
		}
				
		/** batch process 실행 - 페이스북 검색 **/
		List<BatchResponse> results = null;
		try {
			results = getFacebookResults(facebook, nextPage, outFwriter, outFwriterReply);
		} catch (Exception e) {
			e.printStackTrace();
			System.out.println("Facebook query error : "+e.getCause());
			System.exit(-1);
		}
		...
		try { 
			...
			if (!"".equals(nextPage)) { 
				...
				nextPage = nextPage.replace("https://graph.facebook.com/v2.3/","");
				runFacebookPosts(searchID, facebook, false, nextPage, outFwriter, outFwriterReply);
			}
		} catch (Exception ge) { }
		/* -- **/
	}
	...
	public static void runFacebookFeed(String searchID, Facebook facebook, boolean isStart, String nextPage, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws FacebookException, JSONException, IOException {
		/** 시작할 때는 검색 페이지 URL 고정, 그 뒤에는 nextPage를 처리 -> 재귀호출 **/
		if (isStart) { 
			//nextPage = "266715880026269/posts?fields=id,updated_time,name,caption,description,message,likes&limit=20&offset=1";
			nextPage = searchID+"/feed?fields=id,updated_time,name,caption,description,message,likes&limit=20&offset=1";
			//nextPage = searchID+"/links?fields=id,updated_time,name,caption,description,message,likes&limit=20&offset=1";
		}
	...
	public static void runFacebookLinks(String searchID, Facebook facebook, boolean isStart, String nextPage, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws FacebookException, JSONException, IOException {
		/** 시작할 때는 검색 페이지 URL 고정, 그 뒤에는 nextPage를 처리 -> 재귀호출 **/
		if (isStart) { 
			//nextPage = "266715880026269/posts?fields=id,updated_time,name,caption,description,message,likes&limit=20&offset=1";
			//nextPage = searchID+"/posts?fields=id,updated_time,name,caption,description,message,likes&limit=20&offset=1";
			nextPage = searchID+"/links?fields=id,name,created_time,caption,description,message,likes&limit=20&offset=1";
		}
		...
		try { 
			...
			if (!"".equals(nextPage)) { 
				...
				nextPage = nextPage.replace("https://graph.facebook.com/v2.2/","");
				nextPage = nextPage.replace("https://graph.facebook.com/v2.3/","");
				runFacebookLinks(searchID, facebook, false, nextPage, outFwriter, outFwriterReply);
			}
		} catch (Exception ge) { }
		/* -- **/
	}
	...
	public static List<String> getFacebookContentList(Facebook facebook, List<BatchResponse> results, OutputStreamWriter outFwriter, OutputStreamWriter outFwriterReply) throws FacebookException, JSONException, IOException {
		...
		if (joResult.length() > 1) {
		//System.out.println("###"+results.toString()+"::"+joResult.toString()+"::"+joResult.length()+"::"+joResult.isNull("data"));
		...
		for(int i = 0 ; i < item.length(); i++) {
			...
			try {
				String c_date = "";
				if(obj1.isNull("updated_time") != true) { 
					c_date = obj1.get("updated_time").toString();   
					//System.out.println("updated_time ::>> "+c_date);
				} else if (obj1.isNull("created_time") != true) { 
					c_date = obj1.get("created_time").toString();   
					//System.out.println("created_time ::>> "+c_date);
				}
				cc_date = DateUtils.getChangeDate(c_date); 
			} catch (Exception e) {
				//
				e.printStackTrace();
			}
			
			if ("".equals(cc_date)) { cc_date = "_"; }
			...
			String outStr = "";
			if (!"".equals(c_message)) { 
				outStr = lt + siteCode + rt +tag 
						+ lt + uuid + rt +tag 
						+ lt + dataType + rt  +tag 
						+ lt + cc_date + rt +tag
						+ lt + c_id + rt +tag 
						+ lt + c_message + rt +tag
						+ lt + String.valueOf(c_likeCnt) + rt; 
			}
		...
		if (itemArray != null && itemArray.length() > 0) {
			...
			for(int i = 0 ; i < itemArray.length(); i++) {
				...
				if (!"".equals(c_message)) { 
					outStr = lt + siteCode + rt +tag 
							+ lt + uuid  + rt +tag 
							+ lt + dataTypeReply  + rt +tag 
							+ lt + cc_date  + rt +tag
							+ lt + o_id  + rt +tag
							+ lt + c_id  + rt +tag 
							+ lt + c_message  + rt +tag
							+ lt + c_authorId  + rt +tag
							+ lt + c_authorName  + rt +tag
							+ lt + c_likeCnt + rt +"\n"; 
